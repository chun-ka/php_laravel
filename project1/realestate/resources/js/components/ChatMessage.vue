        ...
        <div class="col-md-2 myUser">
            <ul class="user">
                ...
                <li v-for="(user,index) in users" :key="index">
                    <a href="#" style="text-decoration: none" @click.prevent="userMessage(user.id)">
...
<img v-if="msg.user.role==='agent'" :src=" '/upload/agent_images/'+ msg.user.photo "
                   class="userImg" alt="userImg"/>
<img v-else :src=" '/upload/user_images/'+ msg.user.photo " class="userImg" alt="userImg"/>
...
<img v-if="msg.user.role==='user'" :src=" '/upload/user_images/'+ msg.user.photo " class="userImg"
     alt="userImg"/>
<img v-else :src=" '/upload/agent_images/'+ msg.user.photo " class="userImg" alt="userImg"/>

                 </span>
                            <div class="chat-body clearfix">
                                <div class="header clearfix">
                                    <small class="left text-muted">{{ datetime(msg.created_at) }}
                                    </small>
...
export default {
    ...
    methods: {
        ...
        sendMsg() {
            //data gửi về để thực hiện phương thức SendMsg trong ChatControler
            axios.post('/send-message', {receiver_id: this.selectedUser, msg: this.msg})
                .then((res) => {
                    this.msg = '';  //dùng để set lại giá trị rỗng của v-model trong ô input
                    this.userMessage(this.selectedUser);
